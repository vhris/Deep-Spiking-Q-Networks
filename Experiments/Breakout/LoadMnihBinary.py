...
def save_replay_memory(replay_memory, model):
    """Saves the states of the environment in memory_inputs.npz and the corresponding Q-values in memory_outputs.npz
    Args:
    ...
    memory_outputs = model(states).detach().numpy()

    np.savez("bestavgsofar/memory_inputs", memory_inputs)
    np.savez("bestavgsofar/memory_outputs", memory_outputs)
